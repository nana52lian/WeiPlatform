...
function refreshCurrentStatus(requests) {
	...
	$.each(requests, function(i,val){
		 if(i%2==0){
			  if(val.taskStatus==0)
		      	  $("#currentStatus").append("<div class='odd'>用户<span>"+val.userName+"</span>发起请求</div>");
		      else if(val.taskStatus==1||val.taskStatus==2)
		    	  $("#currentStatus").append("<div class='odd'>坐席<span>"+val.serverName+"</span>接受<span>"+val.userName+"</span>的请求</div>");
		     // else if(val.taskStatus==2)
		    	  //$("#currentStatus").append("<div class='odd'>坐席<span>"+val.serverName+"</span>正在处理<span>"+val.userName+"</span>的请求</div>");
		      else if(val.taskStatus==3)
		    	  $("#currentStatus").append("<div class='odd'>坐席<span>"+val.serverName+"</span>关闭<span>"+val.userName+"</span>的请求</div>");
		  }
		  else{
			  if(val.taskStatus==0)
		      	  $("#currentStatus").append("<div class='even'>用户<span>"+val.userName+"</span>发起请求</div>");
		      else if(val.taskStatus==1||val.taskStatus==2)
		    	  $("#currentStatus").append("<div class='even'>坐席<span>"+val.serverName+"</span>接受<span>"+val.userName+"</span>的请求</div>");
		      //else if(val.taskStatus==2)
		    	  //$("#currentStatus").append("<div class='even'>坐席<span>"+val.serverName+"</span>正在处理<span>"+val.userName+"</span>的请求</div>");
		      else if(val.taskStatus==3)
		    	  $("#currentStatus").append("<div class='even'>坐席<span>"+val.serverName+"</span>关闭<span>"+val.userName+"</span>的请求</div>");
		  }
	  });   
}  
...
<body>
<div style="text-align:right"><a href="public_monitor.jsp" class="easyui-linkbutton" >Chinese</a></div>
<div>
    <div style="float: left;width:300px;padding-left: 20px;">
        <div style="font-size: 20px;width:300px;padding-top: 5px;font-weight: bold;color: #2f7ed8;">All user requests：<label id="requestCount"></label></div>
        <div style="font-size: 20px;width:300px;padding-top: 5px;font-weight: bold;color: #2f7ed8;">Active user requests：<label id="userActiveCount"></label></div>
    </div>
