...
<![endif]-->
<![if !IE]>
<audio id="snd" src='${pageContext.request.contextPath}/images/msg.wav' preload="auto">
</audio>
<![endif]>
...
/*  			    ,onSelectRow:function(rowIndex,rowData){
	...
	function transferTask(){
		...
		var serviceSelectRow = $('#tt_service').datagrid("getSelections");
		if(serviceSelectRow.length>0){
			...
			var toUserIds=[];
			//taskIds.push(taskId);
			toUserIds.push(transferToId);
/*  			for(var key in selectRows){
				var leaderId = selectRows[key].id;
				leaderIds.push(leaderId);
			}  */
		var queuerows = $('#tt_uncomplete_advisory').datagrid("getSelections");	
		for(var key in queuerows){
			var taskId = queuerows[key].id;
			taskIds.push(taskId);
		}  
			...
			//$.post("${pageContext.request.contextPath}/weixin!transferTask",{taskID:taskId,leaderIds:JSON.stringify(leaderIds),transferToId:transferToId},function(data){
				$.post("${pageContext.request.contextPath}/weixin!transferTask",{taskID:JSON.stringify(taskIds),leaderIds:JSON.stringify(leaderIds),transferToId:JSON.stringify(toUserIds)},function(data){
				
				
				
				if(data.resDesc == 'Operation Success'){
					...
					//for(var i=0;i<rows.length;i++){ 
						...
						var index = $('#tt_uncomplete_advisory').datagrid("getRowIndex",rows[i]);  //getRowIndex
						$('#tt_uncomplete_advisory').datagrid("deleteRow",index);
