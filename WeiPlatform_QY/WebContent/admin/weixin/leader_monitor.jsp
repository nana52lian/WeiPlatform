...
function refreshPage(){
	$.post("${pageContext.request.contextPath}/weixin!getTodayProcessingCount",function(data){	
	    		...
	    		//横的柱状图
	    		getBar(data.unProcessCount,data.mainQueenTooLongTime);
	    		//曲线图
...
function refreshCurrentStatus(requests) {
	...
	$.each(requests, function(i,val){
		  if(i%2==0){
			  if(val.taskStatus==0){
				  if('<%=session.getAttribute("lang")%>' == "en_US"){
					  $("#currentStatus").append("<div class='odd'>request from user <span>"+val.userName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
					  $("#currentStatus").append("<div class='odd'>用户<span>"+val.userName+"</span>发起请求&nbsp;&nbsp;"+val.timeing+"</div>");
				  }
			  }else if(val.taskStatus==1){
				  if('<%=session.getAttribute("lang")%>' == "en_US"){
					  $("#currentStatus").append("<div class='odd'>request from <span>"+val.userName+"</span> has been dispatched to agent<span>"+val.serverName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
                  ...
                  if('<%=session.getAttribute("lang")%>' == "en_US"){
                	  $("#currentStatus").append("<div class='odd'>agent <span>"+val.serverName+"</span> is now processing request from <span>"+val.userName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
                  ...
                  if('<%=session.getAttribute("lang")%>' == "en_US"){
                	  $("#currentStatus").append("<div class='odd'>agent <span>"+val.serverName+"</span> close the request from <span>"+val.userName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
			  ...
			  if(val.taskStatus==0){
				  if('<%=session.getAttribute("lang")%>' == "en_US"){
					  $("#currentStatus").append("<div class='even'>request from user <span>"+val.userName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
					  $("#currentStatus").append("<div class='even'>用户<span>"+val.userName+"</span>发起请求&nbsp;&nbsp;"+val.timeing+"</div>");
				  }
			  }else if(val.taskStatus==1){			  
				  if('<%=session.getAttribute("lang")%>' == "en_US"){
					  $("#currentStatus").append("<div class='even'>request from <span>"+val.userName+"</span> has been dispatched to agent<span>"+val.serverName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
		    	  ...
		    	  if('<%=session.getAttribute("lang")%>' == "en_US"){
		    		  $("#currentStatus").append("<div class='even'>agent <span>"+val.serverName+"</span> is now processing request from <span>"+val.userName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
					  $("#currentStatus").append("<div class='even'>坐席<span>"+val.serverName+"</span>正在处理<span>"+val.userName+"</span>的请求&nbsp;&nbsp;"+val.timeing+"</div>");
				  }
			  }else if(val.taskStatus==3){
				  if('<%=session.getAttribute("lang")%>' == "en_US"){
					  $("#currentStatus").append("<div class='even'>agent <span>"+val.serverName+"</span> close the request from <span>"+val.userName+"</span>&nbsp;&nbsp;"+val.timeing+"</div>");
				  } else {
...
//横的柱状图
function getBar(unProcessCount,mainQueenTooLongTime){
	var defaultColor = '#8bbc21';
	if(unProcessCount>mainQueenTooLongTime){
		defaultColor = '#c42525';
	}
...
<!-- <div>
   ...
   <div id="pie_container" style="width:300px; height: 200px; float: left;"></div>
</div> -->
<div id="line_container" style="width:500px; height: 250px; float: left;border:0px solid red;"></div>
<div id="pie_container" style="width:400px; height: 250px; float: left;border:0px solid blue;"></div>
